...
const imageStyle = computed(() => {
  if (!mouseInside.value || !parallax.value) {
    return { transform: 'translate3d(0, 0, 0) scale(1.05)' };
  }
  const x = parallax.value.tilt * 20;
  const y = parallax.value.roll * 20;
  return { transform: `translate3d(${x}px, ${y}px, 0) scale(1.1)` };
});
...
<template>
  ...
  <div
    v-if="loading"
    class="relative h-[320px] md:h-[420px] lg:h-[520px] w-full rounded-2xl md:rounded-3xl overflow-hidden"
  >
  ...
  <div
    ...
    ref="container"
    class="relative h-[320px] md:h-[420px] lg:h-[520px] w-full rounded-2xl md:rounded-3xl overflow-hidden cursor-default contain-layout"
    @mouseenter="onMouseEnter"
    @mouseleave="onMouseLeave"
  >
    <!-- 背景图片（视差层） -->
    <div class="absolute inset-0 transition-transform duration-700 ease-out" :style="imageStyle">
      <VenusImage
        :src="bannerImage"
        alt="Hero Banner"
    ...
    <!-- 左侧边缘微光 -->
    <div class="absolute inset-y-0 left-0 w-1 bg-gradient-to-b from-transparent via-mint-400/40 to-transparent" />
    ...
    <div class="absolute inset-0 flex flex-col justify-end p-6 md:p-10 lg:p-12">
      ...
      <!-- 博客标题 -->
      <h1
        class="text-white font-bold leading-tight tracking-tight transition-all duration-700 delay-[80ms] text-shadow-banner text-3xl md:text-4xl lg:text-5xl"
        :class="[
      ...
      <!-- 描述文字 -->
      <p
        class="mt-2 text-white/75 text-sm md:text-base transition-all duration-700 delay-[160ms]"
        :class="titleVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'"
      >
        记录技术旅程，分享编程心得
      </p>
      ...
      <!-- 统计数据 -->
      <div
        class="mt-6 flex items-center gap-3 md:gap-4 transition-all duration-700 delay-[240ms]"
        :class="titleVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4'"
      >
