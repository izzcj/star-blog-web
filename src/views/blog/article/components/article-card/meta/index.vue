...
<template>
  <!-- 精选卡片 -->
  <div v-if="isFeatured" class="flex items-center gap-4 mt-auto">
    <!-- 作者 -->
  ...
  <!-- 普通卡片 -->
  <div v-else class="flex items-center justify-between pt-3 mt-auto border-t border-gray-100">
    <div class="flex items-center gap-2 min-w-0">
      <div v-if="article.createByName" :class="getAvatarClass(false)">
        {{ article.createByName.charAt(0) }}
      </div>
      ...
      <span v-if="article.publishTime" class="text-xs text-gray-400">
        {{ formatRelativeTime(article.publishTime) }}
      </span>
    </div>
