...
<template>
  ...
  <div class="fixed inset-0 flex items-center justify-center overflow-hidden">
    ...
    <div class="relative z-10 flex flex-col items-center gap-4 w-full px-4">
      ...
      <!-- 搜索卡片（玻璃态） -->
      <div
        class="search-card w-full max-w-[680px] bg-white/10 backdrop-blur-[20px] border border-white/20
      ...
      >
        ...
        <div class="flex gap-1.5 mb-4">
          <button
            v-for="e of engines"
            :key="e.key"
            class="px-3.5 py-[5px] rounded-full border text-[13px] cursor-pointer transition-[background,color,border-color] duration-200"
        ...
        <div class="search-wrapper will-change-transform origin-center">
          <div
            ...
            duration-300 motion-reduce:transition-none max-[480px]:h-[50px] max-[480px]:px-3"
            :class="isFocused
      ...
      <button
        ...
        @click="router.push(CommonRouterPath.HOME)"
      >
        进入首页
      </button>
    </div>
